 ...
 */
public class DiscountBeforeTax extends RequirementAdapter {
    public boolean checkRequirement(IConditionFindingManagerUserExit item, IStep step, IAccess access) {
        if (item.getAttributeValue("CASH_DISCOUNT").equals("X")
                && item.getAttributeValue("TAX_BASE_IS_NET").equals("X")) {
            return true;
        }
        return false;
    }
